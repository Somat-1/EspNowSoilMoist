...
// ========== POWER MANAGEMENT ==========
#define SLEEP_DURATION_US (6ULL * 60ULL * 60ULL * 1000000ULL)  // 6 hours in microseconds
#define NUM_MEASUREMENTS  3     // Number of measurements to average before sending
#define DELAY_BETWEEN_MS  1000  // 1 second between measurements

// ========== ESP-NOW CONFIGURATION ==========
// MAC address of the receiver (hub) - TramReader MAC Address
uint8_t hubMacAddress[] = {0x80, 0xF1, 0xB2, 0x50, 0x29, 0x70};
...
void setup() {
  ...
  Serial.println("\n\n=== ESP32-C3 Soil Moisture Sensor (Olga) ===");
  ...
  for (int i = 0; i < NUM_MEASUREMENTS; i++) {
    ...
    // Wait between measurements (except after last one)
  ...
  // Calculate averages
  sensorData.batteryVoltage = batterySum / NUM_MEASUREMENTS;
  sensorData.batteryPercent = calculateBatteryPercent(sensorData.batteryVoltage);
  sensorData.soilMoisture = moistureSum / NUM_MEASUREMENTS;
...
float readBatteryVoltage() {
  ...
  // Apply BAT_CAL calibration factor
  float voltage = (adcValue / 4095.0) * 3.3 * 2.0 * BAT_CAL;
  
  Serial.printf("  Raw ADC: %d, Calibrated Voltage: %.2f V\n", adcValue, voltage);
